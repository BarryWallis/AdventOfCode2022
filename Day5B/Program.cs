...
IInputParser inputParser = new InputParser(textReader);
IList<CrateStack> crateStacks = inputParser.ParseCrateStacks();
ICollection<MoveCommand> moveCommands = inputParser.ParseMoveCommands();
foreach (MoveCommand moveCommand in moveCommands)
{
    CrateStack temporaryCrateStack = new();
    for (int i = 0; i < moveCommand.CratesToMove; i++)
    {
        //crateStacks[moveCommand.To - 1].Push(crateStacks[moveCommand.From - 1].Pop());
        temporaryCrateStack.Push(crateStacks[moveCommand.From - 1].Pop());
    }
    for (int i = 0; i < moveCommand.CratesToMove; i++)
    {
        crateStacks[moveCommand.To - 1].Push(temporaryCrateStack.Pop());
    }
}

StringBuilder resultBuffer = new();
foreach (CrateStack crateStack in crateStacks)
{
    _ = resultBuffer.Append(crateStack.Peek().Name);
}

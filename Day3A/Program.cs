...
{
    List<HashSet<char>> rucksacks = GetRuscksackContents(line);
    IEnumerable<char> intersection = rucksacks[0].Intersect(rucksacks[1]);
...
List<HashSet<char>> GetRuscksackContents(string line)
{
    Guard.IsNotNullOrWhiteSpace(line);
    Guard.IsEqualTo(line.Length % 2, 0);
    Guard.IsTrue(line.All(char.IsLetter));

    List<HashSet<char>> rucksacks = new()
    {
        new HashSet<char>(line[..(line.Length / 2)]),
        new HashSet<char>(line[(line.Length / 2)..])
    };
    return rucksacks;
}

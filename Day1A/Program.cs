...
List<int> elvesCalorieCount = GetElfCalorieCount(Console.In);
int result = elvesCalorieCount.Max();
Console.WriteLine(result);

List<int> GetElfCalorieCount(TextReader textReader)
{
    List<int> result = new();
    while (Console.In.Peek() != -1)
    {
        result.Add(AnElfCalorieCount(textReader));
    }

    return result;
}

int AnElfCalorieCount(TextReader textReader)
{
    string line;
    int calorieCount = 0;
    while ((line = textReader.ReadLine() ?? string.Empty).Trim() != string.Empty)
    {
        try
        {

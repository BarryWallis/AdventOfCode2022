...
{
    (HashSet<int> firstElfAssignment, HashSet<int> secondElfAssignment) = ParseLine(line);
    result += firstElfAssignment.IsSubsetOf(secondElfAssignment)
              || secondElfAssignment.IsSubsetOf(firstElfAssignment)
              ? 1 : 0;
}
Console.WriteLine(result);

(HashSet<int> firstElfAssignment, HashSet<int> secondElfAssignment) ParseLine(string line)
{
    Guard.IsNotNullOrWhiteSpace(line);
    Guard.IsTrue(line.Count(c => c == ',') == 1);

    string[] elfAssignments = line.Split(",");
    IEnumerable<int> firstElfAssignment = GetElfAssignment(elfAssignments[0]);
    IEnumerable<int> secondElfAssignment = GetElfAssignment(elfAssignments[1]);

    return (firstElfAssignment.ToHashSet(), secondElfAssignment.ToHashSet());
}

HashSet<int> GetElfAssignment(string elfAssignmentSource)
{
    Guard.IsNotNullOrWhiteSpace(elfAssignmentSource);
    Guard.IsEqualTo(elfAssignmentSource.Count(c => c == '-'), 1);

    string[] floors = elfAssignmentSource.Split("-");
    Guard.IsEqualTo(floors.Length, 2);

    int rangeStart = int.Parse(floors[0]);
    int rangeEnd = int.Parse(floors[1]);
    Guard.IsTrue(rangeStart <= rangeEnd);

    return Enumerable.Range(rangeStart, rangeEnd - rangeStart + 1).ToHashSet();
}

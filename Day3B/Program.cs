...
For efficiency, within each group of three Elves, the badge is the only item type carried by all three Elves. That
is, if a group's badge is item type B, then all three Elves will have item type B somewhere in their rucksack, 
...
int result = 0;
List<HashSet<char>> lines;
while ((lines = GetRuscksackContents()).Count != 0)
{
    IEnumerable<char> intersection = RucksacksIntersection(lines);
    result += GetItemValue(intersection);
}
Console.WriteLine(result);

IEnumerable<char> RucksacksIntersection(List<HashSet<char>> lines)
{
    Guard.IsNotEmpty(lines);

    IEnumerable<char> result = lines[0];
    foreach (HashSet<char> line in lines.Skip(1))
    {
        result = result.Intersect(line);
    }

    Guard.IsEqualTo(result.Count(), 1);
    return result;
}


int GetItemValue(IEnumerable<char> intersection)
{
    Guard.IsEqualTo(intersection.Count(), 1);
    Guard.IsTrue(char.IsLetter(intersection.First()));

    char item = intersection.First();
    return char.IsLower(item) ? item - 'a' + 1 : item - 'A' + 27;
}

List<HashSet<char>> GetRuscksackContents()
{
    List<HashSet<char>> rucksacks = new();
    if (Console.In.Peek() > -1)
    {
        rucksacks = new()
        {
            new HashSet<char>(Console.ReadLine() ?? throw new EndOfStreamException("Unexpected end of file")),
            new HashSet<char>(Console.ReadLine() ?? throw new EndOfStreamException("Unexpected end of file")),
            new HashSet<char>(Console.ReadLine() ?? throw new EndOfStreamException("Unexpected end of file")),
        };
    }
